...
SELECT nom, année, montant
FROM JOUEUR 
JOIN GAIN ON JOUEUR.idJoueur = GAIN.idJoueur
ORDER BY JOUEUR.nom, GAIN.année;
...
SELECT CONCAT(J.nom, ' ', J.prénom), COUNT(M.idGagnant) AS victoires
FROM JOUEUR J
JOIN MATCH M ON J.idJoueur = M.idGagnant
GROUP BY CONCAT(J.nom, ' ', J.prénom)
