...
class Recette
{
  ...
  public function findAll()
  {
    if ($_SESSION['isAdmin'] ?? false) {
      ...
      $query = "SELECT * FROM recettes WHERE isApproved";
    }
  ...
  public function add($titre, $description, $auteur, $image, $type_plat)
  {
    ...
    $isApproved = $_SESSION['isAdmin'] ?? false ? 1 : 0;

    $query = "INSERT INTO recettes (titre, description, auteur, image, date_creation, type_plat, isApproved)
    VALUES (:titre, :description, :auteur, :image, NOW(), :type_plat, :isApproved)";
    $stmt = $this->conn->prepare($query);
